...
alias gstashunapply='git stash show -p stash@{0} | git apply -R' # Experimental
alias gstashdelete_topmost='git stash drop stash@{0}'
# Home made stashing system
alias gstore='git commit -m "stashed !!!!"'
alias grestore='git reset HEAD~1'       # Removes the stored/stashed commit changes at HEAD (same as gundocommit)
alias gundocommit='git reset HEAD~1'    # Remove current commit at the top of history
